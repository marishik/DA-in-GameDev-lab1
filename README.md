...
Epsilon влияет на быстроту изменения поведения во время обучения. Установка небольшого значения этого параметра приведет к более стабильным обновлениям, но также замедлит процесс обучения. Я повысила значение этого параметра, соответственно скорость обучения у меня увеличилась, а также это привело к к менее стабильным обновлениям

![изображение](https://user-images.githubusercontent.com/114138439/205225394-fed73701-0b26-45d2-8fd0-dc1bd04fda64.png)
...
График *Cumulative Reward*: это среднее совокупное вознаграждение, которое максимизирует агент. Нужно, чтобы оно увеличивалось, если нужна успешная тренировка, но по некоторым причинам оно может и уменьшаться. Всегда лучше максимизировать вознаграждения в интервале от 1 до -1.
...
График *Policy Loss*: Соотносится с тем, насколько сильно меняется процесс принятия решений. Величина этого параметра должна уменьшаться во время успешной тренировки. Эти значения будут колебаться во время тренировки. Как правило, они должны быть меньше 1

График *Value Loss*: Средняя потеря функции обновления значения. Связано с тем, , насколько хорошо модель способна предсказать значение каждого состояния. Этот график должен увеличиваться, пока агент учится, а затем уменьшаться, как только вознаграждение стабилизируется. 



График *Beta*: У всех тренировок одинаковый - идет монотонно вниз. Beta соответствует силе энтропийной регуляризации, что делает политику «более случайной». Это гарантирует, что агенты должным образом исследуют пространство действия во время обучения. Увеличение этого параметра обеспечит выполнение большего количества случайных действий. Это должно быть скорректировано таким образом, чтобы энтропия (измеряемая с помощью TensorBoard) медленно уменьшалась вместе с увеличением вознаграждения.

График *Entropy*: Показывает насколько случайны решения модели. Нужно, чтобы это значение уменьшалось, потому что агент узнаёт больше об окружении и ему нужно меньше исследовать.

График *Epsilon*: во всех случаях уменьшается, влияет на скорость изменения политики во время обучения. При установке небольшого значения этого параметра, обновления будут более стабильными, но процесс обучения также замедлится. Типичный диапазон: 0.1 - 0.3. В моем случае значение было 0,3.
...
График *Extrinsic Value Estimate*: Оценка среднего значения для всех положений, посещенных агентом, которая должна увеличиваться во время успешной тренировки. 

График *Learning Rate*: Отвечает за коэффициент скорости обучения. График скорости обучения изменяет скорость обучения во время обучения и чаще всего меняется между эпохами / итерациями.


## Выводы

В процессе лабораторной работы я познакомилась с интеграцией экономической системы в проект Unity и обучением ML-Agent'а. Также, я познакомилась с TensorBoard. Меняя значения параметров, я наблюдала за обучением ML-Agent'а, и узнавала, какие параметры за что отвечают. Для поиска наиболее эффективного обучения можно менять значения и экспериментрировать с параметрами в .yaml файле, анализировать графики. 
